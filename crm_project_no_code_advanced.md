...
Branch erstellen: Erstelle einen neuen Branch für dieses Ticket.

git checkout -b feature/kunden-liste

Code implementieren (crm.py):
...
Merge in main (nach Absprache mit Lehrer/Mitschülern):
...
Verwende einen try-except-Block, um FileNotFoundError (wenn die Datei noch nicht existiert), json.JSONDecodeError (wenn die Datei kein gültiges JSON enthält) und allgemeine Exceptions abzufangen.
